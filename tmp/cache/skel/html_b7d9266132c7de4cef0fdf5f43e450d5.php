...
((unique(initiale(extraire_multi($Pile[$SP]['nom']))))  ?
		...
		' . ' ' . (	'
		' .
	vide($Pile['vars'][(string)'p'] = concat(table_valeur($Pile["vars"], (string)'p', null),afficher_initiale(ancre_url(parametre_url(self(),'debutaut',(	'@' .
				$Pile[$SP]['id_auteur'])),'paginationaut'),initiale(extraire_multi($Pile[$SP]['nom'])),$Numrows['_lettre']['compteur_boucle'],table_valeur($Pile["vars"], (string)'debut', null),interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'nb', null), '10'),true))))) .
	'
		')) :
		'') .
...
function BOUCLE_liste_authtml_b7d9266132c7de4cef0fdf5f43e450d5(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$l2 = _T('public|spip|ecrire:texte_vide');$SP++;
	// RESULTATS
	while ($Pile[$SP]=$iter->fetch()) {
...
(($t1 = strval(recuperer_numero($Pile[$SP]['titre_rang'])))!=='' ?
		($t1 . '. ') :
		'') .
interdire_scripts(((($a = typo(supprimer_numero($Pile[$SP]['nom']), "TYPO", $connect, $Pile[0])) OR (is_string($a) AND strlen($a))) ? $a : $l2)) .
'</a></td>
			<td class=\'email\'>' .
...
// Temps de compilation total: 26.447 ms
//
...
function html_b7d9266132c7de4cef0fdf5f43e450d5($Cache, $Pile, $doublons=array(), $Numrows=array(), $SP=0) {
	...
	$connect = '';
	$page = (
(($t1 = strval(vide($Pile['vars'][(string)'defaut_tri'] = array('statut' => '1', 'multi nom' => '1', 'site' => '1', 'compteur_articles' => '-1', 'points' => '-1
'))))!=='' ?
		($t1 . '
') :
		'') .
...
' .
		...
		vide($Pile['vars'][(string)'debut'] = interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'debutaut', null), interdire_scripts(eval('return '.'_request("debutaut");'.';'))),true))) .
